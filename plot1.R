...
##  and developing strategies. Here's a good quote from a swirl lesson about 
##  exploratory graphs: "They help us find patterns in data and understand its 
##  properties. They suggest modeling strategies and help to debug analyses. We 
...
## The goal is not to deduct points over small deviations from the requirements or 
## for legitimate differences in implementation styles, etc. Look for ways to 
## give points when it's clear that the submitter has given a good faith effort to do 
## the project, and when it's likely that they've succeeded. Most importantly, it's 
## okay if a person did something differently from the way that you did it. The point 
## is not to see if someone managed to match your way of doing things, but to see if someone objectively accomplished the task at hand.
##
...
##    
##    Create a separate R code file (𝚙𝚕𝚘𝚝𝟷.𝚁, 𝚙𝚕𝚘𝚝𝟸.𝚁, etc.) that constructs 
##    the corresponding plot, i.e. code in 𝚙𝚕𝚘𝚝𝟷.𝚁 constructs the 𝚙𝚕𝚘𝚝𝟷.𝚙𝚗𝚐 plot. 
...
data2 <- data %>% 
  ...
  mutate(DateTime = make_datetime(year = year(Date), month = month(Date), day = day(Date), hour = hour(Time), min = minute(Time), sec = second(Time)))
